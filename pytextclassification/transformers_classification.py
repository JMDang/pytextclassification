...
sys.path.append('..')
from transformer_config.model_args import (
    ClassificationArgs,
)

from transformers_classification_utils import (
    InputExample,
...
class BertClassificationModel:
    def __init__(
        ...
        Args:
            ...
            num_labels: The number of labels or classes in the dataset.
        ...
        MODEL_CLASSES = {
            ...
            "ernie": (ErnieConfig, ErnieForSequenceClassification, BertTokenizerFast),
        }
        ...
        self.loss_fct = init_loss(
            weight=self.weight, device=self.device, args=self.args, multi_label=self.args.multi_label
        )
        ...
        if self.args.onnx:
            ...
            if not self.args.quantized_model:
                ...
                self.model = model_class.from_pretrained(
                    None, state_dict=quantized_weights
                )
    ...
    def train(
        ...
        if self.args.evaluate_during_training and eval_df is None:
            logger.warning(
                ...
                " Pass eval_df to model.train() if using evaluate_during_training."
        ...
        if verbose:
            logger.info(
                " Training of {} model complete. Saved to {}.".format(
                    self.args.model_type, self.args.output_dir
                )
            )
    ...
    def train_cycle(
        ...
        self.args.warmup_steps = (
            warmup_steps if self.args.warmup_steps == 0 else self.args.warmup_steps
        )
        ...
        if self.args.scheduler == "constant_schedule":
            ...
            raise ValueError("{} is not a valid scheduler.".format(self.args.scheduler))
        ...
        train_iterator = trange(
            int(self.args.num_train_epochs), desc="All Train Process", disable=self.args.silent, mininterval=0
        )
        epoch_number = 0
        ...
        if self.args.model_name and os.path.exists(self.args.model_name):
            try:
                # set global_step to gobal_step of last saved checkpoint from model path
                checkpoint_suffix = self.args.model_name.split("/")[-1].split("-")
        ...
        if self.args.wandb_project:
            if not wandb.setup().settings.sweep_id:
                ...
                wandb.run._label(repo="pytextclassification")
        ...
        for _ in train_iterator:
            ...
            batch_iterator = tqdm(
                train_dataloader,
                desc=f"Running Epoch {epoch_number} of {self.args.num_train_epochs}",
            ...
            for step, batch in enumerate(batch_iterator):
                ...
                if show_running_loss:
                    logger.info(f"Running Epoch {epoch_number}/{self.args.num_train_epochs}. Running Loss: {current_loss:9.4f}")
                    # batch_iterator.set_description(
                    #     f"Running Epoch {epoch_number}/{self.args.num_train_epochs}. Running Loss: {current_loss:9.4f}"
                    # )
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0:
                    ...
                    if self.args.logging_steps > 0 and global_step % self.args.logging_steps == 0:
                        ...
                        if self.args.wandb_project or self.is_sweeping:
                            wandb.log(
                                {
                    ...
                    if self.args.evaluate_during_training and (
                        ...
                        if not best_eval_metric:
                            best_eval_metric = results[self.args.early_stopping_metric]
                            self.save_model(
                        ...
                        if best_eval_metric and self.args.early_stopping_metric_minimize:
                            ...
                            if (
                                    results[self.args.early_stopping_metric] - best_eval_metric
                                    > self.args.early_stopping_delta
                            ):
                                best_eval_metric = results[self.args.early_stopping_metric]
                                self.save_model(
            ...
            epoch_number += 1
            output_dir_current = os.path.join(
                self.args.output_dir, "checkpoint-{}-epoch-{}".format(global_step, epoch_number)
            )

            if self.args.save_model_every_epoch:
                self.save_model(output_dir_current, optimizer, scheduler, model=model)

            if self.args.evaluate_during_training and self.args.evaluate_each_epoch:
                results, _, _ = self.eval(
                    eval_df,
                    verbose=verbose and self.args.evaluate_during_training_verbose,
                    silent=self.args.evaluate_during_training_silent,
                ...
                if best_eval_metric and self.args.early_stopping_metric_minimize:
                    if (
                        ...
                        if (
                                self.args.use_early_stopping
                                and self.args.early_stopping_consider_epochs
                        ):
                            if early_stopping_counter < self.args.early_stopping_patience:
                                early_stopping_counter += 1
                                if verbose:
                                    logger.info(
    ...
    def eval(
        ...
        result, model_outputs, wrong_preds = self.eval_cycle(
            eval_df,
            output_dir,
            verbose=verbose,
            silent=silent,
    ...
    def eval_cycle(
        ...
        Utility function to be used by the eval() method. Not intended to be used directly.
        """
        ...
        if self.args.use_hf_datasets:
            eval_dataset = load_hf_dataset(
                eval_df, self.tokenizer, self.args, multi_label=self.args.multi_label
            )
            eval_examples = None
        ...
        preds = np.empty((len(eval_dataset), self.args.num_labels))
        if self.args.multi_label:
            out_label_ids = np.empty((len(eval_dataset), self.args.num_labels))
        else:
        ...
        elif not self.args.multi_label and args.regression is True:
        ...
        if (
            ...
            if not args.labels_map:
                self.args.labels_map = {i: i for i in range(self.args.num_labels)}
    ...
    def load_and_cache_examples(
        ...
        if not self.args.multi_label and args.regression:
    ...
    def predict(self, to_predict):
        ...
        if not self.args.multi_label and self.args.onnx:
            ...
            if self.args.multi_label:
                dummy_label = [dummy_label for i in range(self.args.num_labels)]
            ...
            if not self.args.multi_label and args.regression is True:
                preds = np.squeeze(preds)
                model_outputs = preds
            else:
                model_outputs = preds
    ...
    def load_model(self):
        best_model_file = os.path.join(train_args.best_model_dir, "pytorch_model.bin")
...
if __name__ == "__main__":
    ...
    train_args.lazy_loading = True
    #train_args.use_hf_datasets = True
    ...
    train_df = pd.read_csv("./data/train.txt.part", delimiter="\t")
    ...
    model.train(
            ...
            eval_df="./data/dev.txt",
            verbose=True)
    
    #加载在验证集效果最好的模型进行预测
    model.load_model()
    preds, model_outputs = model.predict(["柔弱的儿媳跪在地上向老头子苦苦哀求：“够了，适可而止！",
